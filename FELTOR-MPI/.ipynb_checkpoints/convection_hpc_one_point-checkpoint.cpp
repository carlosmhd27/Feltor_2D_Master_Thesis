...
int main( int argc, char* argv[])
{
    ...
    int data_1P_IDs[4];
    for( unsigned i=0; i<4; i++){
        err = nc_def_var( ncid, names_1P[i].data(),  NC_DOUBLE, 3, dim_ids, &data_1P_IDs[i]);}
    ...
    ///////////////////////////////////first output/////////////////////////
    //// Things to do:
    //// Eliminate the interpolation
    //// take only the central point, change start and count
//     size_t count[3] = {1, grid_out.n()*grid_out.Ny(), grid_out.n()*grid_out.Nx()};
//     std::cout << "count: " << grid_out.n()*grid_out.Ny() << std::endl;
//     size_t start[3] = {0, 0, 0};
        
    size_t count_1P[] = {1};
    size_t start_1P[] = {0}; // grid_out.n()*grid_out.Nx()
    size_t middle[2] = {p.Ny / 2, p.Nx / 2};
...
//     dg::blas2::symv( interpolate, y0[0], transferD[0]); /// ions and electrons are the same
//     dg::blas2::symv( interpolate, y0[0], transferD[1]); /// ions and electrons are the same
//     dg::blas2::symv( interpolate, exp.potential(), transferD[2]);
//     dg::blas2::symv( interpolate, y0[1], transferD[3]);

	//////////////////////////////////Saving Data//////////////////////
    ///// Field and center
    std::cout << "Start: "<< start_1P[0] << ' ' << start_1P[1] << ' ' << start_1P[2] << std::endl;

    for( int k=0;k<4; k++)
    {
        dg::blas1::transfer( transferD[k], transferH);
//         err = nc_put_vara_double( ncid, dataIDs[k], start, count, transferH.data() );
        err = nc_put_vara_double( ncid, data_1P_IDs[k], start_1P, count_1P, transferH.data() );
    }
    
    ///// Saving time and energy_time
...
//                     dg::blas1::transfer( transferD[i], transferH);
                    err = nc_put_vara_double( ncid, invariantID[i], Estart, Ecount, &exp.invariants()[i]);
                    err = nc_put_vara_double( ncid, dissID[i], Estart, Ecount, &exp.invariants_diffusion()[i]);
                    err = nc_put_vara_double( ncid, data_1P_IDs[i], start_1P, count_1P, transferD[i][middle[0]][middle[1]]);
