...
int main( int argc, char* argv[])
{
    ...
    if (p.save_CP){
        ...
        // dg::blas1::axpby (  1., y0[0],           0., transferD_1P[0]);
        // dg::blas1::axpby (  1., y0[0],           0., transferD_1P[1]);
        // dg::blas1::axpby (  1., exp.potential(), 0., transferD_1P[2]);
        // dg::blas1::axpby (  1., y0[1],           0., transferD_1P[3]);

        // for( int k = 0; k < 4; k++){
        //     dg::blas1::transfer( transferD_1P[k], transferH_1P);
        ...
        for( unsigned j=0; j<p.itstp; j++)
        {
            ...
            if (p.save_CP){start_CP[0] += 1;}
            {
                ...
                for( int i=0; i<4; i++)
                {
                    ...
                    err = nc_put_vara_double( ncid, dissID[i], Estart, Ecount, &exp.invariants_diffusion()[i]);
                    // if (p.save_CP){
                    //     dg::blas1::transfer( transferD_1P[i], transferH_1P);
                    //     err = nc_put_vara_double( ncid, dataIDs_1P[0], start_CP, count_CP, transferH_1P.data() + middle);
                    // }
                }
