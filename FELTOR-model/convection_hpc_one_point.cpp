...
int main( int argc, char* argv[])
{
    ...
    for( unsigned i=0; i<4; i++){
        err = nc_def_var( ncid, names_1P[i].data(),  NC_DOUBLE, 1,  &EtimeID, &data_1P_IDs[i]);
    }
    ...
    const unsigned middle = p.Ny * p.Nx / 2;
    ...
    for( int k=0;k<4; k++)
    {
        ...
        err = nc_put_vara_double( ncid, data_1P_IDs[k], start_1P, count_1P, transferH.data() + middle);
    }
        ...
        for( unsigned j=0; j<p.itstp; j++)
        {
            ...
            start_1P[0] += 1;
            {
                ...
                for( int i=0; i<4; i++)
                {
                    ...
                    err = nc_put_vara_double( ncid, invariantID[i], Estart, Ecount, &exp.invariants()[i]);
                    err = nc_put_vara_double( ncid, dissID[i], Estart, Ecount, &exp.invariants_diffusion()[i]);
                    err = nc_put_vara_double( ncid, data_1P_IDs[i], start_1P, count_1P, transferH.data() + middle);
